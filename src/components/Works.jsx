  ...
  return (
    <motion.div variants={fadeUp} className="mb-12">
      <div className="flex items-start justify-between gap-4 mb-2">
        <p className="text-[#1a1917] text-[16px] font-medium leading-snug">{name}</p>
        <span className="text-[#aaa8a6] text-[11px] shrink-0 mt-1">{category}</span>
      </div>

      <p className="text-[#555452] text-[15px] font-normal leading-[1.9] mb-4">
        {firstSentence}.
      </p>

      <div className="flex flex-wrap gap-x-4 gap-y-1 mb-4">
        {tags.slice(0, 3).map((tag) => (
          <span key={tag.name} className="text-[#aaa8a6] text-[11px]">{tag.name}</span>
        ))}
      </div>
      ...
      <div className="flex gap-6 text-[12px]">
        {source_code_link && !onClick && (
          <button onClick={() => window.open(source_code_link, "_blank")}
            className="text-[#aaa8a6] hover:text-[#1a1917] transition-colors duration-200">
            GitHub ↗
          </button>
        )}
        {onClick && (
      ...
      <div className="flex items-center gap-8 mb-14">
        {filters.map((f) => (
          <button
            ...
            style={{ color: active === f ? "#1a1917" : "#aaa8a6" }}
          >
            {f}
            {active === f && (
              <motion.div
            ...
            {showCapstone && (
              <>
                {showBuilds && (
                  <motion.p
                    variants={fadeUp}
                    className="text-[#aaa8a6] text-[10px] uppercase tracking-[0.25em] mt-4 mb-10"
                  >
